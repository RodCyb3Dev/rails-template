...
Kodeflash is a Rails template with Tailwind CSS by [Rodney H](https://kodeflash.com).
...
```bash
rails new app-name -d postgresql -m https://raw.githubusercontent.com/Rodcode47/railskodeflash/master/template.rb
```
...
If the app fails due to **Segmentation fault**? just run `$ rails server` again.
...
Inspired heavily by [Jumpstart](https://github.com/excid3/jumpstart) from [Chris Oliver](https://twitter.com/excid3/). Credits to him.
